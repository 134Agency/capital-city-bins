...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const floorDropdown1 = document.getElementById("delivery-floor-drp");
  const floorDropdown2 = document.getElementById("pickup-floor-drp");
...
<div class="bins-delivery-date">%DATE%</div>
    ...
    const extraSuppliesTemplate = extraSuppliesList
      .map((item) =>
        createTemplate(suppliesWrapperTemplate, (template) =>
          template.replace("%NAME%", item.name).replace("%AMOUNT%", item.amount)
        )
      )
  ...
  const updatePlanNameAndPriceUI = function () {
    ...
    const cartPlanName = planInCart.querySelector(".cart-item-title")
      .textContent;

    const cartPlanPrice = planInCart.querySelector(".cart-item-price")
      .textContent;
  ...
  const isSamePlan = function (name, weeks) {
    ...
    const planInCartName = planInCart.querySelector(".cart-item-title")
      .textContent;
    const planInCartDuration = planInCart.querySelector(".color-neutral-800")
      .textContent;
